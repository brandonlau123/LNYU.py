...
class MYSQL_Pipeline(object):
  ...
  def __init__(self):
    ...
    self.conn = MySQLdb.connect(host='localhost', user='root', passwd='magma123', db='crawler', charset="utf8", use_unicode=True)
    self.conn.ping(True)
  ...
  def process_item(self, item, spider):
    try:
        ...
        # Insert item into shoesaddictor table.
            ...
            # Insert item into startcow table.
        elif isinstance(item, StarcowItem):	
            self.cursor.execute("INSERT INTO starcow (name, link, date) VALUES (%s, %s, %s)", (item['name'].encode('utf-8'), item['link'].encode('utf-8'), DATE))
			
        self.conn.commit()
		
        # If item name contain below words. Tweet it.
        if ('overkill' in item['name'].encode('utf-8').lower()) or ('copa' in item['name'].encode('utf-8').lower()) or ('purecontrol' in item['name'].encode('utf-8').lower()) or ('yeezy' in item['name'].encode('utf-8').lower()) or ('max' in item['name'].encode('utf-8').lower() and 'atmos' in item['name'].encode('utf-8').lower()) or ('max' in item['name'].encode('utf-8').lower() and 'master' in item['name'].encode('utf-8').lower()) or ('ronnie' in item['name'].encode('utf-8').lower()) or ('fieg' in item['name'].encode('utf-8').lower()) or ('BY2913' in item['name'].encode('utf-8').lower()) or ('flamingo' in item['name'].encode('utf-8').lower()) or ('powerphase' in item['name'].encode('utf-8').lower()):
          ...
          CONSUMER_KEY = 'Hu9ThVbSgJHof6I9D2lxDmNgO'
          CONSUMER_SECRET = 'aoIHUWEAbjRO3M1Vb0pooreFfgaRs7E3kayls6RemfrQ5WnviV'
          ACCESS_TOKEN_KEY = '850612407894462464-gzNEaolGmg7CBAF1BOLKmXdGwwyEkln'
          ACCESS_TOKEN_SECRET = 'zHF8zuLqu5Dpf3QcRtCTtt8zAUdOMlYfFTWycypiZLFFy'
          API = TwitterAPI(CONSUMER_KEY, CONSUMER_SECRET, ACCESS_TOKEN_KEY, ACCESS_TOKEN_SECRET)
          TEXT_TO_TWEET = item['name'] + " " + item['link']
